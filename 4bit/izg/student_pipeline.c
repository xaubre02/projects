...
void const* gpu_computeVertexAttributeDataPointer(
  ...
  return (void const*)((size_t)head->buffer + (size_t)head->offset + (size_t)head->stride * gl_VertexID);
}
void gpu_runVertexPuller(
  ...
  {
    if(puller->heads[i].enabled == 1) output->attributes[i] = (void const *)gpu_computeVertexAttributeDataPointer(&puller->heads[i], gpu_computeGLVertexID(puller->indices, vertexShaderInvocation));
...
void gpu_runPrimitiveAssembly(
  ...
  for(size_t i = 0, n = baseVertexShaderInvocation; i < nofPrimitiveVertices; i++, n++) 
  {
      gpu_runVertexPuller(&output, puller, n);
      input.attributes = &output;
      input.gl_VertexID = gpu_computeGLVertexID(puller->indices, n);
...
void gpu_computeScreenSpaceBarycentrics(
  ...
  // Bod P náleží trojúhelníku ABC, pokud pro barycentrické souřadnice x,y,z platí:
  // P = xA + yB + zC
  // x = 1 - y - z

  // celá část = hodnota čísla bez jakékoliv proměnné (bez x, y či z)
  // -- Ypsilonová souřadnice bodu P
  float cely = pixelCenter->data[1] - vertices[0].data[1]; //  P - A
  float y =     vertices[1].data[1] - vertices[0].data[1]; // By - Ay
  float z =     vertices[2].data[1] - vertices[0].data[1]; // Cz - Az
  // vyjádření y z rovnice cely = y + z
  // y = cely/y - z/y 
  float y_cely = cely / y; // celá část ypsilonu
  float y_z = 0 - z / y;   // část se z

  // -- iXová souřadnice bodu P
  cely = pixelCenter->data[0] - vertices[0].data[0]; //  P - A
  y =     vertices[1].data[0] - vertices[0].data[0]; // By - Ay
  z =     vertices[2].data[0] - vertices[0].data[0]; // Cz - Az
  // roznásobení vyjádřeného y z předchozí části aktuálním y
  cely = cely - y * y_cely; // celá část
  z = z + y * y_z; // pouze část u proměnné z
  
  // výsledek
  coords->data[2] = cely / z;
  coords->data[1] = y_cely + y_z * coords->data[2];
  coords->data[0] = 1 - coords->data[1] - coords->data[2];
}
/// @}
