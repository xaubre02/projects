...
*/
	if(s == NULL)
	{
		stackError(SERR_INIT);	// volání chybové funkce a skončení
		return;
	}
	else
	{
		s->top = -1; // inicializace zásobníku na prázdný
	}
}
...
*/
	return s->top == (STACK_SIZE - 1) ? 1 : 0; // při indexování od 0 musí obsahovat o 1 méně než je maximum
}
...
** funkci stackEmpty.
*/
	if(stackEmpty(s)) // kontrola podmínky a volání chybové funkce
	{
		stackError(SERR_TOP);
		return;
	}
	...
	{
		*c = s->arr[s->top]; // uložím hodnotu prvku na vrcholu zásobníku
	}
}
...
** 
*/
	if(stackEmpty(s)) // kontrola podmínky a případné ukončení
	{
		return;
	}
	else
	{
		s->top--; // snížím vrchol zásobníku o jeden prvek
	}
}
...
*/
	...
	{
		s->top++; // zvýšení vrcholu zásobníku
		s->arr[s->top] = c; // uložení znaku na novou pozici
	}
}
